...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-header bg-white py-3">
        <strong class="fs-6"><i class="bi bi-clock-history text-primary me-2"></i>Riwayat Perhitungan Kelas</strong>
    </div>
    <!-- Card Body -->
    <div class="card-body">
                ...
                <tbody>
                    <?php if (!empty($riwayat)): ?>
                        ...
                        <?php foreach ($riwayat as $i => $r): ?>
                            <?php $no = $offset + $i + 1; ?>
                            <!-- Highlight Row -->
                            <tr class="<?= $idPerhitungan === (int)$r['id_perhitungan'] ? 'table-primary' : '' ?>">
                                <!-- Checkbox -->
                                ...
                                <td class="text-center col-aksi">
                                    <?php
                                    $lihatUrl = url('perhitungan/kelas')
                                        . '&tahun_ajaran=' . urlencode($r['tahun_ajaran'])
                                        . '&kelas='        . urlencode($r['kelas'])
                                        . '&semester='     . urlencode($r['semester_target'] % 2 === 1 ? 'ganjil' : 'genap')
                                        . '&id='           . $r['id_perhitungan'];
                                    $detailUrl = url('perhitungan/detail-kelas') . '&id=' . $r['id_perhitungan'];
                                    ?>
                                    <!-- Tombol Aksi -->
                                    <div class="d-flex justify-content-center align-items-center">
                                        ...
                                        <button type="button" class="btn btn-sm btn-outline-danger px-2"
                                                onclick="konfirmasiHapus(<?= $r['id_perhitungan'] ?>, '<?= e($r['kelas']) ?>', '<?= tglId($r['tanggal_hitung'], $bulanId) ?>')">
